...
if(!class_exists('medel_Gradient_Param'))
{
    class medel_Gradient_Param
    {
        ...
        function number_settings_field($settings, $value)
        {
            ...
            $output = '<select id="orientation-'.$uni.'">';
            	if(isset($orientation) && !empty($orientation)) {
            		if($orientation == 'horizontal') {
            			$output .= '<option value="horizontal" selected="selected">'.esc_html__('Horizontal', 'medel').'</option>';
            		} else {
            			$output .= '<option value="horizontal">'.esc_html__('Horizontal', 'medel').'</option>';
            		}
				    
            		if($orientation == 'vertical') {
            			$output .= '<option value="vertical" selected="selected">'.esc_html__('Vertical', 'medel').'</option>';
            		} else {
            			$output .= '<option value="vertical">'.esc_html__('Vertical', 'medel').'</option>';
            		}
            ...
            $output .= '<input type="hidden" class="grad-val-'.esc_attr( $uni ).' wpb_vc_param_value ' . esc_attr( $param_name ) . ' ' . esc_attr( $type ) . ' ' . esc_attr( $class ) . ' pt_gradient" name="' . esc_attr( $param_name ) . '"  value="'.esc_attr( $value ).'" '.$dependency.'/>';

            ?>
            	<link rel="stylesheet" href="<?php echo plugins_url('pt-medel-shortcodes') . '/include/css/gradient.css' ?>" type="text/css" media="all">
        ...
        function admin_scripts($hook){
            wp_register_script("classygradient", plugins_url('pt-medel-shortcodes') . '/include/js/jquery.gradient.js',array('jquery'));
...
if(class_exists('medel_Gradient_Param'))
{
    $medel_Gradient_Param = new medel_Gradient_Param();
}