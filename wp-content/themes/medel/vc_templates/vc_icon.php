...
if(!empty($custom_background_gradient)) {
	$custom_background_gradient = strip_tags(explode("||", $custom_background_gradient)[1]);
}
$style = '';
if ( 'custom' === $background_color || $background_color == 'gradient' ) {
	if ( false !== strpos( $background_style, 'outline' ) ) {
		$style = 'border-color:' . $custom_background_color;
	} elseif($custom_background_gradient) {
		$style = $custom_background_gradient;
	} else {
...
<div <?php echo implode( ' ', $wrapper_attributes ); ?>
	...
	} if($background_color == 'gradient' && $custom_background_gradient) {
		echo ' vc_gradient-icon';
	} ?>">
	<div class="vc_icon_element-inner vc_icon_element-color-<?php echo esc_attr( $color );
	if ( $has_style ) {
		echo ' vc_icon_element-have-style-inner';
	} ?> vc_icon_element-size-<?php echo esc_attr( $size ); ?> vc_icon_element-style-<?php echo esc_attr( $background_style ); ?> vc_icon_element-background-color-<?php echo esc_attr( $background_color ); ?>" style="<?php echo esc_attr($style) ?>">
		<?php if($background_color == 'gradient' && $custom_background_gradient) { ?>
			<span class="bor" <?php echo strip_tags($style) ?>></span>
		<?php } ?>
		<span class="vc_icon_element-icon <?php echo esc_attr($iconClass); ?>" <?php echo( 'custom' === $color ? 'style="color:' . esc_attr( $custom_color ) . ' !important"' : '' ); ?>></span><?php
